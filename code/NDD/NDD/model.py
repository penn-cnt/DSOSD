...
class NDD():
    def __innit__(self, hidden_size = 10, fs = 128,
    ...
    def _prepare_segment(self, data, ret_time=False):
        ...
        data_mat = torch.zeros((nwins,j,(self.train_win+self.pred_win),len(data_ch)))
        for k in range(len(data_ch)): # Iterating through channels
            samples = MovingWinClips(data_np[:,k],self.fs,1,0.5)
    ...
    def generate_loss(self, X):
        ...
        X_z = self._scaler_transform(X)
        input_data,target_data, time_wins = self._prepare_segment(X_z)
        ...
        self.loss_df = pd.DataFrame(loss_mat.T,columns = X.columns)
        return self.loss_df
    ...
    def predict_gamma(self, confidence_threshold = 0.95, pre_buffer = 120, post_buffer = 120):
        ...
        assert(hasattr(self,'loss_df'), "Must generate loss before extracting channels")

        sz_prob = self.loss_df.copy()
        ...
        self.sz_prob = gamma.cdf(self.sz_prob.flatten(),a=shape,scale=scale).reshape(sz_prob.shape)[:,onset_idx:offset_idx]
